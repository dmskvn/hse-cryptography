...
AES::AES(std::unique_ptr<IAESContraints> c, std::vector<uint8_t> key)
    :_c(std::move(c))
{
    KeyExpansion(std::move(key));
}

void AES::KeyExpansion(std::vector<uint8_t> key)
{
    _round_key.resize(236);
    unsigned i, j, k;
    uint8_t tempa[4]; // Used for the column/row operations

      // The first round key is the key itself.
      for (i = 0; i < _c->get_nk(); ++i)
      {
        _round_key[(i * 4) + 0] = key[(i * 4) + 0];
        _round_key[(i * 4) + 1] = key[(i * 4) + 1];
        _round_key[(i * 4) + 2] = key[(i * 4) + 2];
        _round_key[(i * 4) + 3] = key[(i * 4) + 3];
      }

      // All other round keys are found from the previous round keys.
      for (i = _c->get_nk(); i < _nb * (_c->get_nr() + 1); ++i)
      {

          k = (i - 1) * 4;
          tempa[0]=_round_key[k + 0];
          tempa[1]=_round_key[k + 1];
          tempa[2]=_round_key[k + 2];
          tempa[3]=_round_key[k + 3];
          ...
          tempa[0] = tempa[0] ^ Rcon[i/_c->get_nk()];
        }
        if(_c->get_type() == 3)
        {
            if (i % _c->get_nk() == 4)
        ...
        j = i * 4;
        k=(i - _c->get_nk()) * 4;
        _round_key[j + 0] = _round_key[k + 0] ^ tempa[0];
        _round_key[j + 1] = _round_key[k + 1] ^ tempa[1];
        _round_key[j + 2] = _round_key[k + 2] ^ tempa[2];
        _round_key[j + 3] = _round_key[k + 3] ^ tempa[3];
        //std::cout << j << std::endl;
      }
}
...
void AES::Decrypt(std::vector<uint8_t>& data)
{
    ...
    AddRoundKey(_c->get_nr());

    // There will be Nr rounds.
    // The first Nr-1 rounds are identical.
    // These Nr rounds are executed in the loop below.
    // Last one without MixColumns()
    for (round = (_c->get_nr() - 1); ; --round)
    {
        InvShiftRows();
        InvSubBytes();
        AddRoundKey(round);
...
void AES::AddRoundKey(uint8_t round)
{
    for (uint8_t i = 0; i < 4; ++i)
    {
        for (uint8_t j = 0; j < 4; ++j)
        {
            (*_state)[i][j] ^= _round_key[(round * _nb * 4) + (i * _nb) + j];
        }
    }
}
